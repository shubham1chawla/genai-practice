...
if __name__ == "__main__":
    ...
    graph.build_graph().invoke({
        ...
        'books_json_path': os.path.join(dirname, 'harry-potter-books', 'books.json'),     # Path to books.json file (Ex. Harry Potter)
        'export_dir': os.path.join(dirname, 'harry-potter-books', 'exports'),             # Export directory for saving LLM's output & metadata (Ex. Harry Potter)
        ...
        'neo4j_import_dir': os.path.join(dirname, 'db', 'import')                         # Neo4j database's import directory (docker volume)
    })
